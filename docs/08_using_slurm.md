...
Instead of running a heavy analysis directly on the machine you log into, you describe your job in a small script — how much CPU, memory and time it needs, and the commands to run — and **submit** it to SLURM. SLURM then queues your job and runs it on a compute node when suitable resources are free.

{: .note }
> This page is a short, cluster-agnostic primer. For detailed, worked examples on the NHM HPC — including Conda, shared software and GPU jobs — see the [Using the job scheduler (Slurm)](04_accessing_hpc_resources/nhm_hpc.html#using-the-job-scheduler-slurm) section of the NHM HPC guide.
...
| `sacct -j <job_id>` | View details of a finished job (including how much memory and time it actually used). |
| `sinfo` | Check the state of the cluster's nodes and partitions. |

## Partitions (queues)

Clusters divide their compute nodes into **partitions** (sometimes called queues), often based on how long a job is allowed to run — for example `hour`, `day`, `week` and `month` on the NHM HPC. As a general rule, shorter jobs are scheduled sooner. Choose the shortest partition that comfortably fits your job.
...
If in doubt about the right resources for a job, ask on your cluster's support channel (see [Getting help](07_getting_help.html)).
